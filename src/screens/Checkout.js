...
const Checkout = observer(({ navigation }) => {
  ...
  const [paymentFee, setPaymentFee] = useState(10000)
  const [shippingCost, setShippingCost] = useState(10000)


  const initializeState = async () => {
    try {
      setSubtotal(items.reduce((sum, item) => sum + item.real_price, 0));
  ...
  useEffect(() => {
    ...
    setItems(carts.map((value) => value.product))
          ...
          >
            <Picker.Item label={'Pilih Pembayaran'} value={''} />
            {
              payments.map(payment => {
                return (
                  <Picker.Item label={payment.paymentName} value={payment.paymentName} key={payment.paymentMethod} />
                )
              })
            }
          </Picker>
